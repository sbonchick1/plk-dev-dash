...
def build_xlsx(payload):
    ...
    chart.add_data(Reference(ws, min_col=2, min_row=3, max_row=data_end), titles_from_data=True)
    ...
    dl = DataLabelList()
    dl.showVal = True
    dl.showCatName = dl.showSerName = dl.showPercent = dl.showLegendKey = False
    dl.position = "outEnd"
    ser.dLbls = dl
    ...
    for ri,s in enumerate(sites,2):
        vals = [s.get("sipId",""),s.get("restNum",""),s.get("fz",""),s.get("address",""),
                s.get("city",""),s.get("state",""),s.get("status",""),s.get("fzOpenDate",""),
                s.get("plkOpenDate",""),s.get("riskLevel",""),s.get("lastComment","")]
